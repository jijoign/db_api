...
class CRUDItem(CRUDBase[models.Item, schemas.ItemCreate, schemas.ItemUpdate]):
    ...
    def get_available_items(self, db: Session, skip: int = 0, limit: int = 100) -> List[models.Item]:
        """Get all available items."""
        return (
            db.query(models.Item)
            .filter(models.Item.is_available == True)
            .offset(skip)
            .limit(limit)
            .all()
        )
    
    def search_by_title(self, db: Session, title: str, skip: int = 0, limit: int = 100) -> List[models.Item]:
